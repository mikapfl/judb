"""Driving a paused debuggee: frames, breakpoints, interrupts and signals.

All over the real websocket, without a browser. Four groups:
...
from helpers import has_error, read_pty_for_url, recv_type, wait_for_exit, ws_url
...
def test_select_frame_retargets_console():
    ...
    async def flow() -> None:
        async with ClientSession() as session, session.ws_connect(ws_url(url)) as ws:
            ...
            r1 = await recv_type(ws, "cell_result")
            assert has_error(r1["outputs"], "NameError")

            # Retarget to the outer frame.
            await ws.send_json({"cmd": "select_frame", "index": outer_idx})
            sel = await recv_type(ws, "frame_selected")
            ...
            await ws.send_json({"cmd": "execute_cell", "code": "marker"})
            r2 = await recv_type(ws, "cell_result")
...
def test_local_does_not_leak_across_frames():
    ...
    async def flow() -> None:
        async with ClientSession() as session, session.ws_connect(ws_url(url)) as ws:
            ...
            await ws.send_json({"cmd": "execute_cell", "code": "secret"})
            r2 = await recv_type(ws, "cell_result")
            assert has_error(r2["outputs"], "NameError")
...
def test_complete_uses_selected_frame_namespace():
    ...
    async def flow() -> None:
        async with ClientSession() as session, session.ws_connect(ws_url(url)) as ws:
            ...
            comp = await recv_type(ws, "completions")
...
def test_expand_returns_repr_and_children():
    ...
    async def flow() -> None:
        async with ClientSession() as session, session.ws_connect(ws_url(url)) as ws:
            ...
            exp = await recv_type(ws, "expanded")
            ...
            nested = await recv_type(ws, "expanded")
...
def test_terminal_ctrl_c_while_paused_ends_the_debuggee(tmp_path: Path):
    ...
    seen: list[str] = []
    try:
        url = read_pty_for_url(master, seen)
